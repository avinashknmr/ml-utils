...
class Metrics:
    def __init__(self, actuals, predictions):
        ...
        self.df = pd.DataFrame({'actuals': actuals, 'predictions': predictions})
    ...
    def _get_auc_threshold(self):
        fpr,tpr,threshold = roc_curve(self.actuals,self.predictions) 
    ...
    def _get_precision_recall_threshold(self):
        precision, recall, thresholds = precision_recall_curve(self.actuals, self.predictions)
    ...
    def gini(self):
        ...
        """
        fpr, tpr, _ = roc_curve(self.actuals, self.predictions)
        auroc = auc(fpr, tpr)
        gini = 2*auroc - 1
        return float(gini)
    ...
    def precision_recall_f1_score(self):
        ...
        self.y_pred=np.where(self.predictions>=threshold_cutoff,1,0)
        tn, fp, fn, tp = confusion_matrix(self.actuals, self.y_pred).ravel()
        precision = precision_score(self.actuals, self.y_pred)
        recall = recall_score(self.actuals, self.y_pred)
        f1 = f1_score(self.actuals, self.y_pred)
    ...
    def decilewise_counts(self, bins=10, cutpoints=None):
        ...
        if cutpoints is None:
            cutpoints = self.df['predictions'].quantile(np.arange(0,bins+1)/bins).reset_index(drop=True)
            cutpoints = sorted(list(set([0]+list(cutpoints)[1:-1]+[1])))
        self.df['bins'] = pd.cut(self.df['predictions'], cutpoints, duplicates='drop')
        out_df = self.df.groupby('bins')['actuals'].agg(['count','sum']).sort_values(by=['bins'], ascending=False).reset_index()

...
def preprocess(data):
    ...
    oe_cols = [c for c in cat_cols if data[c].nunique()>5]
    ...
    preprocess.fit(data)
    cat_columns = preprocess.named_transformers_['categorical']['cat_preprocess'].get_feature_names(ohe_cols)
    columns = np.append(num_cols, cat_columns)

    return preprocess, columns